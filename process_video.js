...
const HEIC_CONVERTED_IMAGEMAGICK =
	"HEIC converti en JPEG en utilisant ImageMagick";
...
const IMAGE_FROM_DIR_INFO =
	"🖼️ Fichier image (le plus récent du répertoire): {imagePath}";
const OUTPUT_DIR_INFO =
	"📁 Répertoire de sortie (même chemin que l'image): {outputDir}";
...
function findMostRecentFile(directoryPath) {
	try {
		...
		console.error(
			`Erreur lors de la recherche du fichier le plus récent: ${error.message}`,
		);
		return null;
	}
}
...
async function main() {
	...
	if (fs.existsSync(imagePath) && fs.statSync(imagePath).isDirectory()) {
		...
		if (mostRecentFile) {
			console.log(
				`Utilisation du fichier le plus récent dans le répertoire: ${path.basename(mostRecentFile)}`,
			);
			imagePath = mostRecentFile;
			imageFromDirectory = true;
		} else {
			console.error(
				`Aucun fichier image trouvé dans le répertoire: ${imagePath}`,
			);
			process.exit(1);
		}
	}
